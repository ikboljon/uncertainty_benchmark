...
from models.sviresnet import svi_resnet18, svi_resnet50
...
def main(data_flag, output_root, num_epochs, gpu_ids, batch_size, download, model_flag, resize, as_rgb, model_path, run):
    ...
    if model_flag == 'sviresnet18':
        model = svi_resnet18(num_classes=n_classes)
    elif model_flag == 'sviresnet50':
        model =  svi_resnet50(num_classes=n_classes)
    else:
    ...
    if model_path is not None:
        ...
        train_metrics = test(
            model, train_evaluator, train_loader_at_eval, task, 
            criterion, device, run, svi_train_samples, kl_weight, 
            output_root
        )
        val_metrics = test(
            model, val_evaluator, val_loader, task, 
            criterion, device, svi_val_samples, kl_weight, 
            run, output_root
        )
        test_metrics = test(
            model, test_evaluator, test_loader, task, 
            criterion, device, run, svi_test_samples, kl_weight, 
            output_root
        )
    ...
    for epoch in trange(num_epochs):        
        train_loss = train(
            model, train_loader, task, criterion, 
            optimizer, device, writer, kl_weight, 
            svi_train_samples
        )
    ...
    train_metrics = test(
        best_model, train_evaluator, train_loader_at_eval, task, 
        criterion, device, run, svi_train_samples, kl_weight, 
        output_root
    )
    val_metrics = test(
        best_model, val_evaluator, val_loader, task, 
        criterion, device, run, svi_val_samples, kl_weight, 
        output_root
    )
    test_metrics = test(
        best_model, test_evaluator, test_loader, task, 
        criterion, device, run, svi_test_samples, kl_weight, 
        output_root
    )
...
def test(model, evaluator, data_loader, task, criterion, device, run, svi_samples, kl_weight, save_folder=None):
